        ...
        <div class="main-content font-size-13">
            <div class="tab__box">
                <div class="tab__items">
                    ...
                    <a class="tab__item is-active" href="<?= url('admin/users/active-customers') ?>">کاربران تایید شده</a>
                </div>
            </div>
                ...
                <table class="table">
                    ...
                    <?php
                        foreach($ActiveCustomers as $ActiveCustomer){ ?>
                            ...
                            <td>1.1.1.1</td>
                            <?php if($ActiveCustomer['is_active'] == 1){ ?>
                            <td class="text-error">تایید نشده</td>
                            <?php }else if($ActiveCustomer['is_active'] == 2){ ?>
                            <td class="text-success">تایید شده</td>
                            <?php } ?>
                            <td>
                                <a href="<?= url('admin/customers/delete/' . $ActiveCustomer['id']) ?>" class="item-delete mlg-15" title="حذف"></a>
                                <a href=" <?= url('admin/customers/confirm/' . $ActiveCustomer['id']) ?>" class="item-confirm mlg-15" title="تایید"></a>
                                <a href="<?= url('admin/customers/not-confirm/' . $ActiveCustomer['id']) ?>" class="item-reject mlg-15" title="رد"></a>
                                <a href="edit-user.html" target="_blank" class="item-eye mlg-15" title="مشاهده"></a>
                                <a href="<?= url('admin/profile/customer/edit/' . $ActiveCustomer['id']) ?>" class="item-edit " title="ویرایش"></a>
                            </td>
                        </tr>
                        <?php } ?>
                    </tbody>
